<?php 
	...
	try { 
		...
		$lignesbeneficiaire1 = $m_mstiersliste->fetchAllBeneficiaireAvecListe();
		...
		if(($lignesbeneficiaire1 == NULL) && ($lignesbeneficiaire != NULL) && (count($lignesbeneficiaire) > 0) && ($lignesdvente != NULL)) {
			
		//if(($lignesbeneficiaire != NULL) && (count($lignesbeneficiaire) > 0) && ($lignesdvente != NULL)) {

		    //if(($lignesbeneficiaire != NULL) && (count($lignesbeneficiaire) > 0) && ($lignesdvente != NULL)) {
		    $nbre_lignesbeneficiaire = count($lignesbeneficiaire);
            //$nbre_lignesbeneficiaire = sizeof($lignesbeneficiaire);
		    while($i < 10) {
				...
				//if($lignedvente->getCode_membre() != "" && $lignedvente->getCode_membre() != NULL) {
			    //$findallocationcmfh = $m_allocation_cmfh->fetchAllAllocationCmfhAvecListe($lignedvente->getCode_membre());
			    //} else {
				//$findallocationcmfh = true;	
				//}
				...
				if((count($rowseumembre) <= 0) && ($finddepotvente) && ($findCodesBySous != NULL) && ($lignesdetfcaps != null) && ($cumulfcaps >= $mont_caps) && ($lignedvente->getSolde_depot() >= $mont_caps) && ($lignebeneficiaire->code_agence != NULL)) {
					...
					if($findsmsmoney == NULL && $souscription->souscription_type == 'BAn') {
                        ...
                        $sms_money->setCode_Agence(null)
                                  ...
                                  ->setSentTo($lignebeneficiaire->portable_membre)
                                  ->setMotif('CAPS')
                                  ->setId_Utilisateur(null)
                                  ->setCurrencyCode('XOF')
    ...
    if($lignebeneficiaire->code_agence != "001001001001") {
        ...
        $m_membreasso->find($lignebeneficiaire->utilisateur, $membreasso);
		...
		$db->rollback();
		//$message = "";
		print $message." ______ ".$apporteur." ".$nom_benef." ".$prenom_benef." ".$i;
		flush();
		return;
    }
