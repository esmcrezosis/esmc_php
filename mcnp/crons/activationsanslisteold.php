<?php 
	...
	try { 
        ini_set('memory_limit', '512000000000000000000000000000000M');
        ...
        $lignesbeneficiaire = $m_mstiersliste->fetchAllBeneficiaireSansListe();
			
	    $dvente   = new Application_Model_EuDepotVente();
        $m_dvente = new Application_Model_EuDepotVenteMapper();
	    $lignesdvente = $m_dvente->fetchAllCmfhSansListe();
		
		
		if(($lignesbeneficiaire != NULL) && (count($lignesbeneficiaire) > 0) && ($lignesdvente != NULL)) {
		...
		$prenom_benef = $lignebeneficiaire->getPrenom_membre();
		$prenom_benef = str_replace("'", " ", $prenom_benef);
		$tabprenom = explode(" ",$prenom_benef);
					
		$nom_benef = $lignebeneficiaire->getNom_membre();
		...
		$selection->where("LOWER(REPLACE(date_nais_membre, ' ', '')) LIKE ? ", strtolower(str_replace(" ", "", $lignebeneficiaire->getDate_nais_membre())));
	        $selection->where("LOWER(REPLACE(lieu_nais_membre, ' ', '')) LIKE ? ", strtolower(str_replace(" ", "", $lignebeneficiaire->getLieu_nais_membre())));
	        $selection->limit(1);
		...
		$lignedvente = $lignesdvente[$j];
                ...
                while($lignedvente->getCode_membre() == "") {
		     $lignedvente = $lignesdvente[$j+1];
		     $j++;
		}

                $findallocationcmfh = $m_allocation_cmfh->fetchAllAllocationCmfhAvecListe($lignedvente->getCode_membre());
                while($findallocationcmfh != false) {
		     $lignedvente = $lignesdvente[$j+1];
		     $findallocationcmfh = $m_allocation_cmfh->fetchAllAllocationCmfhAvecListe($lignedvente->getCode_membre());
		     $j++;
		}
                                
		while($lignedvente->getSolde_depot() < $mont_caps) {
		     $lignedvente = $lignesdvente[$j+1];
		     $j++;
		}
		...
		$id_depot = $lignedvente->getId_depot();
				...
				if((count($rowseumembre) <= 0) && ($finddepotvente) && ($findCodesBySous != NULL) && ($lignesdetfcaps != null) && ($cumulfcaps >= $mont_caps) && ($lignedvente->getSolde_depot() >= $mont_caps)) {
...
//////////////////////////////////////////
if($lignebeneficiaire->getUtilisateur() != 1) {
        $souscription = new Application_Model_EuSouscription();
