...
class Application_Model_EuAncienGcpMapper {
    ...
    public function getDbTable() {
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_EuAncienGcp');
        }
        return $this->_dbTable;
    }

    public function save(Application_Model_EuAncienGcp $gcp) {
        $data = array(
    ...
    public function update(Application_Model_EuAncienGcp $gcp) {
        $data = array(
            'id_gcp' => $gcp->getId_gcp(),
            'code_tegc' => $gcp->getCode_tegc(),
            'id_credit' => $gcp->getId_credit(),
    ...
    public function find($id_gcp, Application_Model_EuAncienGcp $gcp) {
    ...
    public function findGcp($compte) {
        ...
        foreach ($result as $row) {
            $entry = new Application_Model_EuAncienGcp();
...
//////////////////////////////////////////////////////////	
    ...
    public function findByCodeCatSolde($code_membre, $code_cat, Application_Model_EuAncienGcp $Gcp) {
    ...
    public function findOne($code_membre, Application_Model_EuAncienGcp $Gcp) {
        $select = $this->getDbTable()->select();
		if(isset($code_membre) && $code_membre!=""){
        $select->where('code_membre = ?', $code_membre);}
		$select->limit(1);
