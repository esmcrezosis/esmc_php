 ...
 * @author user
 */
class Application_Model_EuAncienCnncMapper {
...
//put your code here
    ...
    public function getDbTable() {
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_EuAncienCnnc');
        }
        return $this->_dbTable;
    }

    public function find($id_cnnc, Application_Model_EuAncienCnnc $cnnc) {
    ...
    public function findsum() {
	    $t_cnnc = new Application_Model_DbTable_EuAncienCnnc();
        $select = $t_cnnc->select();
        $select->from($t_cnnc, array('SUM(mont_credit) as somme'));
	...
	public function findCreditByCompte() {
        $table = new Application_Model_DbTable_EuAncienCnnc();
        $select = $table->select();
        $select->where('mont_credit > ?', 0);
        ...
        foreach ($result as $row) {
            $entry = new Application_Model_EuAncienCnnc();
            $entry->setId_cnnc($row->id_cnnc)
                  ->setCode_membre($row->code_membre)
                  ->setDatefin($row->datefin)
                  ->setLibelle($row->libelle)
    ...
    public function save(Application_Model_EuAncienCnnc $cnnc) {
        $data = array(
    ...
    public function update(Application_Model_EuAncienCnnc $cnnc) {
        $data = array(
        'id_cnnc' => $cnnc->getId_cnnc(),
        'code_membre' => $cnnc->getCode_Membre(),
        'datefin' => $cnnc->getDatefin(),
