...
class Application_Model_EuMembreMoraleMapper {
	...
	public function getLastCodeMembreAdmin()  {
	   $select = $this->getDbTable()->select();
       $select->from($this->getDbTable(), array('MAX(code_membre_morale) as code'));
...
/////////////////////////////////////////////////////////////
 ...
 ->setDesactiver($row->desactiver)
    ...
    public function fetchAllByTableauBordNombre($code_membre_morale = "", $raison_sociale = "", $id_filiere = "", $id_canton = 0, $id_prefecture = 0, $id_region = 0, $id_pays = 0, $code_zone = "", $code_type_acteur = "", $code_statut = "", $quartier_membre = "", $ville_membre = "", $date_identification1 = "", $date_identification2 = "", $code_agence = "", $domaine_activite = 0, $auto_enroler = "", $etat_membre = "") {
        $select = $this->getDbTable()->select();
        $select->from($this->getDbTable(), array('COUNT(code_membre_morale) as COUNT'));
...
WHERE EXISTS (
    ...
    public function fetchAllAll() {
           $select = $this->getDbTable()->select();
            $select->from($this->getDbTable(), array('code_membre_morale', 'raison_sociale', 'date_identification', 'heure_identification'));
            $select->order(array('raison_sociale ASC'));
            //$select->limit(500);
           $resultSet = $this->getDbTable()->fetchAll($select);
        $entries = array();
