...
class Application_Model_EuMembreMapper {
	...
	public function detail($num_membre){
		   ...
		   $entries = array();
                 ...
                 $entries['codesecret']=$row->codesecret;
    ...
    public function save(Application_Model_EuMembre $membre) {
        $data = array(
		   ...
		   'desactiver' => $membre->getDesactiver(),
    ...
    public function fetchAllByTableauBordNombre($code_membre = "", $nom_membre = "", $sexe_membre = "", $id_canton = 0, $id_prefecture = 0, $id_region = 0, $id_pays = 0, $code_zone = "", $profession_membre = "", $formation = "", $quartier_membre = "", $ville_membre = "", $date_identification1 = "", $date_identification2 = "", $code_agence = "", $id_religion_membre = 0, $auto_enroler = "", $etat_membre = "") {
        $select = $this->getDbTable()->select();
        $select->from($this->getDbTable(), array('COUNT(code_membre) as COUNT'));
...
WHERE EXISTS (
    ...
    public function fetchAllAll() {
           $select = $this->getDbTable()->select();
            $select->from($this->getDbTable(), array('code_membre', 'nom_membre', 'prenom_membre', 'date_identification', 'heure_identification'));
            $select->order(array('nom_membre ASC', 'prenom_membre ASC'));
            //$select->limit(199990, 100000);
           $resultSet = $this->getDbTable()->fetchAll($select);
        $entries = array();
        foreach ($resultSet as $row) {
            $entry = new Application_Model_EuMembre();
    ...
    public function fetchAllByFIFO($limit = 0, $limit2 = 0) {
        $select = $this->getDbTable()->select();
        $select->where('etat_membre = ?', 'N');
		$select->where('auto_enroler = ?', 'N');
        $select->where('desactiver IN (?)', array(0,2));
		$select->where('valider is null');
		$select->where('code_agence is not null');
        //$select->where("code_membre NOT IN (SELECT code_membre_beneficiaire FROM eu_mstiers_listebc WHERE type_liste LIKE 'SansListe')");
        $select->order(array('date_identification ASC', 'heure_identification ASC'));
        $select->limit($limit, $limit2);
        $resultSet = $this->getDbTable()->fetchAll($select);
        $entries = array();
        foreach ($resultSet as $row) {
            $entry = new Application_Model_EuMembre();
