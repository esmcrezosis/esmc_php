 ...
 * @author user
 */
class Application_Model_EuCapaMapper {
...
//put your code here
    ...
    public function getDbTable() {
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_EuCapa');
        }
        return $this->_dbTable;
    }

    public function find($code_capa, Application_Model_EuCapa $capa) {
        ...
        $capa->setCode_capa($row->code_capa)
                ...
                ->setHeure_capa($row->heure_capa)
                ->setMontant_capa($row->montant_capa)
    ...
    public function fetchAll() {
        ...
        foreach ($resultSet as $row) {
            $entry = new Application_Model_EuCapa();
    ...
    public function update(Application_Model_EuCapa $capa) {
        $data = array(
            'code_capa' => $capa->getCode_capa(),
            'id_operation' => $capa->getId_operation(),
            'date_capa' => $capa->getDate_capa(),
	...
	public function CumulBAiByMembre($code_membre,$type_capa) {
        $select = $this->getDbTable()->select();
	...
	public function fetchAllBAiByMembre($code_membre,$type_capa) {
	   $tabela = new Application_Model_DbTable_EuCapa();
	   $select = $tabela->select();
	   $select->where('code_membre like ?',$code_membre);
	   $select->where('type_capa like ?',$type_capa);
	   $select->where('origine_capa like ?',"BAI");
       $select->order('date_capa asc');	   
    ...
    public function fetchAllByMembreOrigine($code_membre,$origine_capa) {
       $tabela = new Application_Model_DbTable_EuCapa();
       $select = $tabela->select();
       $select->where("code_membre like '%".$code_membre."%'");
       $select->where('origine_capa like ?',$origine_capa);
       $select->order(array('date_capa desc', 'heure_capa desc'));    
       $result = $tabela->fetchAll($select);
       if(count($result) == 0) {
          return NULL;
       }
