 ...
 * @author Akamgil
 */
class Application_Model_EuSmsmoneyMapper {
    ...
    public function getDbTable() {
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_EuSmsmoney');
        }
        return $this->_dbTable;
    }

    public function save(Application_Model_EuSmsmoney $sms) {
        $data = array(
            'neng' => $sms->getNEng(),
    ...
    public function update(Application_Model_EuSmsmoney $sms) {
        $data = array(
            'neng' => $sms->getNEng(),
            'fromaccount' => $sms->getFromAccount(),
            'destaccount' => $sms->getDestAccount(),
    ...
    public function find($Neng, Application_Model_EuSmsmoney $sms) {
	...
	public function findSMSMoneyByCodeSMS($code_sms, Application_Model_EuSmsmoney $sms) {
        $select = $this->getDbTable()->select();
	...
	public function findlinedestaccount($destaccount_consumed) {
        $select = $this->getDbTable()->select();
        $select->where('destaccount_consumed LIKE ?', $destaccount_consumed);
        $results = $this->getDbTable()->fetchAll($select);
        if(0 == count($results)) {
            return false;
        }
	...
	public function findBySouscription($id_souscription) {
        $select = $this->getDbTable()->select();
        $select->where('destaccount_consumed like ?', $id_souscription);
		$select->where(' creditamount > ?', 70000);
        $results = $this->getDbTable()->fetchAll($select);
        if (count($results) > 0) {
            $row = $results->current();
            $sms = new Application_Model_EuSmsmoney();
