...
class Application_Model_EuArticleStockesMapper {
    ...
    public function fetchAllByReference($code_membre_morale) {
        $select = $this->getDbTable()->select();
        $select->from($this->getDbTable(), array('id_article_stockes', 'code_barre', 'date_enregistrement', 'categorie', 'type', 'designation', 'prix', 'publier', 'reference', 'code_membre_morale', 'vendu', 'imageArticle', 'remise', 'COUNT(reference) as counter'));
        $select->where("code_membre_morale = ? ", $code_membre_morale);
        $select->where("vendu = ? ", 0);
    ...
    public function fetchAllByVendu($code_membre_morale) {
        $select = $this->getDbTable()->select();
        $select->where("code_membre_morale = ? ", $code_membre_morale);
        $select->where("vendu = ? ",1);
        $resultSet = $this->getDbTable()->fetchAll($select);
