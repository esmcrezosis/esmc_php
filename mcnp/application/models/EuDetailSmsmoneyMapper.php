...
class Application_Model_EuDetailSmsmoneyMapper {
    ...
    public function getDbTable() {
        if (NULL === $this->_dbTable) {
            $this->setDbTable('Application_Model_DbTable_EuDetailSmsmoney');
        }
        return $this->_dbTable;
    }

    public function save(Application_Model_EuDetailSmsmoney $detail) {
        $data = array(
    ...
    public function update(Application_Model_EuDetailSmsmoney $detail) {
        $data = array(
            'id_detail_smsmoney' => $detail->getId_detail_smsmoney(),
            'num_bon' => $detail->getNum_bon(),
            'code_membre' => $detail->getCode_membre(),
    ...
    public function find($id_detail_smsmoney, Application_Model_EuDetailSmsmoney $detail) {
	...
	public function findSum($code_membre) {
		...
		$select->where('origine_sms IN (?)',array('MF','PBF'));
    ...
    public function fetchAll() {
        ...
        foreach ($resultSet as $row) {
            $entry = new Application_Model_EuDetailSmsmoney();
	...
	public function findcumul($code_membre,$type_bnp) {
	       $table = new Application_Model_DbTable_EuDetailSmsmoney();
           $select = $table->select();
	...
	public function getSumByProduit($membre,$produit) {
	    $table = new Application_Model_DbTable_EuDetailSmsmoney();
        $select = $table->select();
        $select->from($table,array('SUM(solde_sms) as somme'));
        $select->where('code_membre like ?', $membre);
        $select->where('type_sms like ?', $produit);
		$select->where('solde_sms > ?', 0);
        $result = $this->getDbTable()->fetchAll($select);
        if (count($result) == 0) {
            return 0;
        }
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    public function findByOrigineSmsSolde($code_membre, $origine_sms, Application_Model_EuDetailSmsmoney $DetailSmsmoney) {
        $table = new Application_Model_DbTable_EuDetailSmsmoney();
        $select = $table->select();
