 ...
 */
class EuFsController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $menu = "<li><a href=\" /eu-fs/new \">Nouveau</a></li>";
    ...
    public function newAction() {
        ...
        $form = new Application_Form_EuFs();
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($request->getPost())) {
               $param = new Application_Model_EuParametres($form->getValues());
               $mapper = new Application_Model_EuParametresMapper();
               $count = $mapper->findConuterfs();
                   if ($count == 1) {
                      $message = 'Le paramétrage du frais de solvabilité est déjà défini.';
