 ...
 */
class EuPeriodeController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $menu = "<li><a href=\" /eu-periode/new \">Nouveau</a></li>";
    ...
    public function dataAction() {
        ...
        $select->where('code_param = ?','periode');
    ...
    public function newAction()  {
        ...
        $form = new Application_Form_EuPeriode();
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($request->getPost())) {
               $param = new Application_Model_EuParametres($form->getValues());
               $mapper = new Application_Model_EuParametresMapper();
               $count = $mapper->findConuterperiode();
                   if ($count == 1) {
                      $message = 'Le paramétrage de la période  est déjà défini.';
